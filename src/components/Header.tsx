...
export const Header = (): JSX.Element => {
  ...
  return (
    <header className="header">
      <div
        className={clsx('header-container', pathname !== '/' && 'header-container-scrolled')}
        ref={headerContainerRef}
      >
        <div className="header-content d-flex justify-between items-center" ref={headerContentRef}>
          <h1>
            <a className="logo-content d-flex items-center" id="homepage-link" href="/">
            ...
            {headerActionListItems.map((action) => (
              <li className="action-item" key={action?.id}>
                <Button
                  ...
                  label={<IconWrapper iconClass={clsx('header-icon', action?.iconClass)} />}
                  href={action?.href}
                />
